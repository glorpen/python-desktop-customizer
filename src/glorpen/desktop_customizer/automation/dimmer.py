...
class Dimmer(object):
    ...
    def dim_others(self, used_window):
        ...
        for output in window_resources.outputs:
            ...
            if x >= crtc_info.x and y >= crtc_info.y and x < crtc_info.x + crtc_info.width and y < crtc_info.y + crtc_info.height:
    ...
    def _find_fullscreen(self, window):
        ...
        for w in r.children:
            d = self.conn.core.GetProperty(window=w, delete=False, type=xcffib.xproto.Atom.ATOM, property=self._ATOM_NET_WM_STATE, long_offset=0, long_length=4).reply()
            if d.length:
                z = b"".join(d.value)
                atoms = struct.unpack("I"*int(len(z)/4), z)
                if set([self._ATOM_FULLSCREEN, self._ATOM_FOCUSED]).issubset(atoms):
                    return w
    ...
    def is_fullscreen(self, window):
        try:
            d = self.conn.core.GetProperty(window=window, delete=False, type=xcffib.xproto.Atom.ATOM, property=self._ATOM_NET_WM_STATE, long_offset=0, long_length=4).reply()
        ...
        if d.length:
            z = b"".join(d.value)
            atoms = struct.unpack("I"*int(len(z)/4), z)
            return set([self._ATOM_FULLSCREEN, self._ATOM_FOCUSED]).issubset(atoms)
        return False
    ...
    def _walk_windows(self, window, cb):
        ...
        d = self.conn.core.GetProperty(window=window, delete=False, type=xcffib.xproto.Atom.Any, property=self._ATOM_WM_STATE, long_offset=0, long_length=4).reply()
